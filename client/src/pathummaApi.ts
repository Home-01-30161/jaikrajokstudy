...
export async function analyzeHomework(imageBlob: Blob): Promise<VisionResult> {
  ...
  try {
    llmReply = await callTextLLM(
      ...
      `3. **สรุปคำตอบสุดท้าย (สรุป 1 ครั้งเท่านั้น!)**: ระบุตัวเลือกที่ถูกต้องอย่างชัดเจน ปิดท้ายด้วย **คำตอบที่ถูกต้องคือ: [ตัวเลือก]** เพียงครั้งเดียว\n` +
      `กฎสำคัญ: (1) ห้ามตัดคำตอบกลางคันเด็ดขาด! เขียนให้ครบถ้วนสมบูรณ์ทุกส่วน (2) ห้ามแสดงส่วนสรุปซ้ำกันมากกว่า 1 ครั้งเด็ดขาด`,
      MATH_SYSTEM_PROMPT,
      6000,
      0.05
    );
