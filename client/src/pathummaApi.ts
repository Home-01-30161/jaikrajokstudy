...
export async function analyzeHomework(imageBlob: Blob): Promise<VisionResult> {
  ...
  try {
    const rawReply = await callTextLLM(
      `ข้อความที่ OCR อ่านได้จากภาพทุกตัวอักษร:\n"${answer.slice(0, 1200)}"\n\n` +
      `⚠️ กฎเหล็ก:\n` +
      `- ใช้เฉพาะข้อความที่ OCR ให้มาข้างต้นเท่านั้น ห้ามเพิ่ม/แก้ไข/ตีความข้อความตัวเลือก ก. ข. ค. ง. เอง\n` +
      `- ก่อนวิเคราะห์แต่ละตัวเลือก ให้คัดลอกข้อความตัวเลือกนั้นจาก OCR มาแสดงก่อน (ใส่เครื่องหมายคำพูด)\n\n` +
      `วิธีตอบ:\n` +
      `1. **วิเคราะห์ตัวเลือก ก.**: คัดลอก "ข้อความ ก. จาก OCR" → ถูกหรือผิด? → เหตุผลสั้น\n` +
      `2. **วิเคราะห์ตัวเลือก ข.**: คัดลอก "ข้อความ ข. จาก OCR" → ถูกหรือผิด? → เหตุผลสั้น\n` +
      `3. **วิเคราะห์ตัวเลือก ค.**: คัดลอก "ข้อความ ค. จาก OCR" → ถูกหรือผิด? → เหตุผลสั้น\n` +
      `4. **วิเคราะห์ตัวเลือก ง.**: คัดลอก "ข้อความ ง. จาก OCR" → ถูกหรือผิด? → เหตุผลสั้น\n` +
      `5. **สรุปคำตอบ (1 ครั้งเท่านั้น)**: ปิดท้ายด้วย **คำตอบที่ถูกต้องคือ: [ก./ข./ค./ง.]** โดยอ้างอิงเฉพาะเนื้อหาตัวเลือกที่คัดลอกมาข้างต้น`,
      MATH_SYSTEM_PROMPT,
      3072,
      0.0
    );
